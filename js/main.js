...
// 스크롤 값에 따라 상호작용하는 요소들 
const interactItem = document.querySelectorAll(".interact");
const interactPoint = [400, 500, 800, 1000, 2100, 2400, 2700]; // 스크롤 위치값

window.addEventListener("scroll", e=>{
  var scroll = window.pageYOffset;
  // console.log(scroll);
  interactPoint.forEach((el, i) => {
    if(scroll >= el) interactItem[i].classList.add("active");
  });
...
function autoBanner(){
  ...
  activation(slideRollList,slideIdx,"on");
  autobnn = setTimeout(autoBanner, 5000);
  /* 인터발은 재귀함수로 넣을 경우 오토배너가 n*2번씩 실행됨.
  타임아웃과 인터발 차이 알기. 
  */
}
})